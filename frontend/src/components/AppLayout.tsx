...
function NavList({ onNavigate }: { onNavigate?: () => void }) {
 ...
 return (
  <List sx={{ px: 1.5, py: 1 }}>
   {nav.map((item) => {
    const active = item.to === "/" ? loc.pathname === "/" : loc.pathname === item.to || loc.pathname.startsWith(`${item.to}/`);
    return (
     <ListItemButton
      key={item.to}
      component={NavLink}
     ...
     >
      ...
      <ListItemText primary={item.label} primaryTypographyProps={{ fontWeight: active ? 700 : 500, fontSize: "0.8125rem" }} />
     </ListItemButton>
    );
   })}
  </List>
...
export function AppLayout() {
 ...
 return (
  <Box sx={{ display: "flex", minHeight: "100vh", bgcolor: "background.default" }}>
   ...
   >
    <Toolbar>
     <IconButton color='inherit' edge='start' onClick={() => setMobileOpen(true)} sx={{ mr: 2, display: { md: "none" } }} aria-label='open menu'>
      <MenuIcon />
     </IconButton>
     <Typography
      variant='subtitle1'
     ...
     <Box
      sx={{
       ...
       border: `1px solid ${health.isLoading ? alpha("#64748b", 0.25) : online ? alpha("#059669", 0.35) : alpha("#dc2626", 0.35)}`,
      }}
     >
      <Box
       sx={{
        ...
        boxShadow: (t) => `0 0 0 3px ${alpha(health.isLoading ? t.palette.grey[400] : online ? t.palette.success.main : t.palette.error.main, 0.25)}`,
       }}
      />
      <Typography variant='caption' fontWeight={700} color={health.isLoading ? "text.secondary" : online ? "success.dark" : "error.dark"}>
